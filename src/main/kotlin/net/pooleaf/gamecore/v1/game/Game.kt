package net.pooleaf.gamecore.v1.game
...
import net.pooleaf.gamecore.v1.events.game.*
import net.pooleaf.gamecore.v1.map.GameMap
import net.pooleaf.gamecore.v1.phase.PhasePipeline
import net.pooleaf.gamecore.v1.phase.PhaseTask
import net.pooleaf.gamecore.v1.player.GamePlayer
import net.pooleaf.gamecore.v1.sql.dtos.GameParticipantDto
import net.pooleaf.gamecore.v1.sql.dtos.GameWinnerDto
import net.pooleaf.gamecore.v1.sql.dtos.toDto
...
abstract class Game {
    ...
    open suspend fun init() {
        ...
        // 투표 초기화
        GameCore.startVoteManager.initVote()
        GameCore.mapVoteManager.initVote()
    ...
    open fun canStart(): Boolean {
        return initialized
                && !GameCore.game.isCountingStarted
    ...
    fun start(starter: CommandSender?): Boolean {
        if (isCountingStarted) return false
     ...
     */
    fun end(): Boolean {
        if (!isCountingStarted || !canEnd() || isEnded) return false
    ...
    open fun onPlayerLeft() {
        ...
        else if (GameCore.game.isCountingStarted && !GameCore.game.isEnded
            && ((GameCore.game.isGameStarted && GameCore.teamManager.getNotDefeatedOnlineTeams().size <= 1)
                    || (!GameCore.game.isGameStarted && GameCore.playerManager.getOnlinePlayingPlayers().size <= 1))) {
            BukkitAsyncScope.launch {
