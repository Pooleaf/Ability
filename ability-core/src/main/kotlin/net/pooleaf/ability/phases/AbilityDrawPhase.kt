...
class AbilityDrawPhase: Phase() {
    ...
    override suspend fun onStart() {
        BukkitBroadcaster.broadcastActionBar("§e잠시 후 능력 추첨이 시작됩니다.")
        ...
        skipJob = BukkitAsyncScope.launch {
            ...
            for (count in drawSkipSeconds!! downTo 1) {
                ...
                if (count <= 10) {
                    ...
                    BukkitBroadcaster.broadcast("${remainingTime} §e후 능력이 강제로 확정됩니다.")
                    BukkitBroadcaster.broadcastSound(XSound.UI_BUTTON_CLICK, 0.3F, 0.7F)
                }
            ...
            BukkitBroadcaster.broadcast("§e모든 플레이어의 능력이 강제로 확정되었습니다.")
            BukkitBroadcaster.broadcastSound(XSound.UI_BUTTON_CLICK, 0.3F, 0.7F)
        }
    }
    ...
    override fun onEnd() {
        BukkitBroadcaster.broadcastActionBar("§e모든 플레이어가 능력을 확정했습니다.")
