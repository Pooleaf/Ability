...
@AutoRegisterExclude
class AbilityWarListener: Listener {
    ...
    private fun getAbilities(ab: AbilityBase): List<AbilityWarCompatAbility> {
        return AbilityApi.unsafe.abilityManager.getAssignedAbilities()
            .filter { it is AbilityWarCompatAbility }
            .filter { it.name == ab.GetAbilityName() }
            .map { it as AbilityWarCompatAbility }
            .toList()
    }
